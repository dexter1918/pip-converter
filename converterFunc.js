...
function postToIn(st) //Postfix to Infix Converter Function
{
    ...
    for (x in st) {
        if ((st[x].charCodeAt(0) >= 65 && st[x].charCodeAt(0) <= 90) || (st[x].charCodeAt(0) >= 97 && st[x].charCodeAt(0) <= 122)) {
            ...
            count += 1;
        } else if (count >= 2) { //aphabets getting popped on operator encounter
            var ch1 = ar[count - 1];
            var ch2 = ar[count - 2];
            ar.pop();
            ar.pop();
...
function inToPost(st) //Infix to Postfix converter
{
    ...
    for (x in st) {
        ...
        } else if (st[x] == ")") //when a closing bracket is encountered
        {
            ...
            while (count > 0 && ar1[count - 1] != "\0" && ar1[count - 1] != "(") { //adding all the operators of stack to the string until an opening bracket is encountered in the stack
                ar2 = ar2.concat(ar1[count - 1]);
                ar1.pop();
                ar3.pop();
                count -= 1;
            }
            if (ar1[count - 1] == "(") { //popping the opening bracket left
                ar1.pop();
                ar3.pop();
                count -= 1;
            }
        } else if (st[x] == "^") { //^ operator gets the highest priority
            if (x == 0 || st[x - 1] == "^" || st[x - 1] == "*" || st[x - 1] == "/" || st[x - 1] == "+" || st[x - 1] == "-" || st[x - 1] == "(")
                f = -1;
            ar1.push(st[x]);
            ar3.push(3);
            count += 1;
        } else if (st[x] == "*" || st[x] == "/") { //* and / operator get the highest priority after ^
            if (st[x - 1] == "^" || st[x - 1] == "*" || st[x - 1] == "/" || st[x - 1] == "+" || st[x - 1] == "-" || st[x - 1] == "(")
                f = -1;
            ar1.push(st[x]);
            ar3.push(2);
            count += 1;
        } else if (st[x] == "+" || st[x] == "-") { //+ and - operator get the least priority but more than brackets
            if (st[x - 1] == "^" || st[x - 1] == "*" || st[x - 1] == "/" || st[x - 1] == "+" || st[x - 1] == "-" || st[x - 1] == "(")
                f = -1;
            ar1.push(st[x]);
            ar3.push(1);
        ...
        if (f < 0)
            break;
        if (ar3[count - 1] != 0 && ar3[count - 2] != 0 && count > 1) { //if there are more than 1 operators in the stack and the top 2 of them are not parentheses
            while (true) {
    ...
    if (f == 0) {
        while (count > 0) { //popping the remaining operators in the stack and putting them into the string
            ar2 = ar2.concat(ar1[count - 1]);
            ar1.pop();
            ar3.pop();
            count -= 1;
...
function inToPre(st) //infix to prefix converter
{
    ...
    for (x in ar1) { //the opening brackets are converted to closing ones and vice versa
        if (ar1[x] == "(")
            ar1[x] = ")";
        else if (ar1[x] == ")")
            ar1[x] = "(";
